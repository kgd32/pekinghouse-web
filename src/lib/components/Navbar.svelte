...
<header class="w-full h-16 bg-black sticky top-0 z-50">
	<div class="container px-4 md:px-0 h-full mx-auto flex justify-between items-center">
		<!-- Logo Here -->
		<a class="text-white text-xl font-lemonmilkbold" href="/">PEKING HOUSE</a>
		...
		>
			<li class="md:hidden fixed top-4 right-6">
				<button class="text-right text-white text-4xl" on:click={toggleNavbar}>&times;</button>
			</li>
			...
			<li>
				<a href="/releases" class="text-white opacity-70 hover:opacity-100 duration-300  "
					>Releases</a
				>
			</li>
			<li>
		...
		{#if showMobileNav}
			<ul
				...
				class="{showMobileNav
					? 'w-full h-screen'
					: 'hidden'} fixed z-50 top-0 right-0 px-10 py-16 bg-black
                font-lemonmilkbold"
